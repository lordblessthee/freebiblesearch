...
$title="Search Result";
require_once("template/default.header.inc.php");
	...
	else
	{
	  ...
	  while($row=fgetcsv ($kjvdb, 8000, ","))
	  {
		  //print_r($row);
				$verseNo=$row[2];
				//$chapterNo = $row[1];
				echo $verseNo."<br>"; $verseText=$classGrepSearch->allStrReplaceTag(htmlentities(html_entity_decode($row[3])),$template['searchResult']['Verse']['SearchKeyStartTag'],
				$template['searchResult']['Verse']['SearchKeyEndTag']);
				//echo $verseText;
				//$newLine =eval("return \"".$template['searchResult']['Verse']['ProcessHTML']."\";");
				//eval("\$newLine=".$template['searchResult']['Verse']['ProcessHTML']."\";");
				echo $template['searchResult']['Verse']['ProcessHTML'];
				//echo $newLine;
