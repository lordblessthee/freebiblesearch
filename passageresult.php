...
<form method="post" action="passageresult.php" name="lookup">
	...
	foreach ($bibleVersionArray as $bibVersion) {
		...
		foreach ($bibleVerseParseInfo as $parseInfo) {
			...
			if ($databaseType == "DB") {
				...
				// Handling for the last chapter (endChap), only if it's different from startChap
				if ($parseInfo['startChap'] != $parseInfo['endChap']) {
					$verseTextArray = getChaptersFromDB($databaseInfo, $bookName, $parseInfo['endChap']);
					$fileContentsStruct[] = array(
						$parseInfo['endChap'],
						array_slice($verseTextArray, 0, $parseInfo['endVerse'])  // Slice up to the endVerse
					);
	...
	{
		global $diagnosticMessage, $Book, $preview, $BibleVersion, $versions, $lookupString, $sampleBibleVersion, $sampleLookupString, $bibleVersionArray;
		$versionfound = false;
